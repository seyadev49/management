...
export const TenantDetailsModal: React.FC<TenantDetailsModalProps> = ({
  ...
  if (tenant.termination_notes) {
    try {
      terminationDetails =
        typeof tenant.termination_notes === 'string'
                            ...
                            >
                              ...
                              <div>
                                ...
                                <div>
                                  {tenant.days_until_next_payment! <= 0
                                    ? 'Overdue'
                                    : `${tenant.days_until_next_payment} days`}
                                </div>
                              </div>
                            </div>
                          )}
                ...
                <div className="space-y-4">
                  ...
                  {terminationDetails && (
                    <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                      ...
                      {terminationDetails.deductions &&
                        terminationDetails.deductions.length > 0 && (
                          <div className="mb-3">
                            <span className="text-gray-600 dark:text-gray-400 block mb-2">
                              Deductions:
                            </span>
                            <div className="space-y-1">
                              {terminationDetails.deductions.map(
                                (deduction: any, index: number) => (
                                  <div
                                    key={index}
                                    className="flex justify-between items-center text-sm bg-red-50 dark:bg-red-900/20 p-2 rounded"
                                  >
                                    <span className="text-gray-900 dark:text-gray-100">
  ...
  {activeTab === 'active' && tenant.contract_status === 'active' && (
    <div className="flex space-x-3">
      ...
      {/* ✅ Only show "Renew Contract" if within 60 days of expiry */}
      {tenant.days_until_expiry !== null && tenant.days_until_expiry <= 60 && (
        <button
          onClick={() => {
            onClose();
            onRenew(tenant);
